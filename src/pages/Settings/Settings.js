...
const Settings = () => {
  ...
  const { dataConf, loadingConf, errorConf, listConfig, modifySettings, createConfig, setDefaultValues } = useSettings();
  ...
  useEffect(()=>{
    if(errorConf?.codeError === "CONF010"){ //Codigo de usuario no existente
        setDefaultValues();
        createConfig(user.id);
    }
  },[user.id, errorConf, createConfig, setDefaultValues]);
